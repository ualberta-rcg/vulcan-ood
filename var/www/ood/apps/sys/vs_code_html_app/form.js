...
const minCpuField = document.getElementById('batch_connect_session_context_min_cpu');
const minRamField = document.getElementById('batch_connect_session_context_min_ram');
...
function toggleGpuFields() {
  ...
  const showGpu = gpuCheckbox.is(':checked');

  console.log('--- toggleGpuFields() called ---');
  console.log('gpuCheckbox checked?:', showGpu);
  console.log('Current gpuData:', gpuData);
  console.log('Available GPU IDs:', Object.keys(gpuData.gpu_name_mappings));

  if (showGpu) {
    console.log('GPU checkbox checked, populating GPU dropdown...');
    updateGpuTypeDropdown();   // (populate dropdown)
    updateGpuCountMax();        // <--- **ADD THIS HERE**!
    gpuType.parent().show();
    gpuCount.parent().show();
  } else {
    console.log('GPU checkbox unchecked, clearing GPU dropdown...');
...
function toggleMemtask() {
  ...
  const checked = memtaskCheckbox.is(':checked');
