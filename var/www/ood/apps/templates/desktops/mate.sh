#!/bin/bash
# mate.sh — self-contained MATE desktop launcher for OOD VNC apps.
...
log(){ echo "[desktop:mate] $*"; }
log "starting MATE session for $USER at $(date)"
...
export XDG_CURRENT_DESKTOP=MATE DISABLE_SYSTEMD=1 GIO_USE_VFS=local GTK_MODULES="" PIPEWIRE_DISABLE=1
...
# --- MATE desktop cosmetics ---
gsettings set org.mate.caja.desktop computer-icon-visible false 2>/dev/null || true
gsettings set org.mate.caja.desktop trash-icon-visible false 2>/dev/null || true
...
# --- default browser = Chrome (MATE reads mimeapps.list) ---
...
# --- MATE session (starts its own window manager, marco) ---
log "launching mate-session"
mate-session &
SESSION_PID=$!
sleep 2
ps -p "$SESSION_PID" >/dev/null || { log "ERROR: mate-session failed to start"; exit 1; }
